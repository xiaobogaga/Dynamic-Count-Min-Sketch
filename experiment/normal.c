...
int i8_normal_ab(double a, double b, long long int *seed)
...
double* generateDoubleNormalData(int n, double low, double high) {
	int i = 0;
	double* data = (double*) malloc(sizeof(double) * n); 
	long* seed = (long*)malloc(sizeof(long) * n);
	while (i < n) {
		seed[i] = getSystemTime() + i * 13;
		data[i] = r8_normal_01(&seed[i]);
		i++;
	}
	free(seed);
	return data;
...
void freeDoubleNormalData(double* data) {
	free(data);
}

/*
	return a positive normal data distribution.
*/
int* generateIntNormalData(int low, int high, int size) {
	int* data = (int*)malloc(sizeof(int) * size);
	long* seed = (long*)malloc(sizeof(long) * size);
	int s = getSystemTime();
	// float* d = r8vec_normal_01_new(size, &s);
	memset(data, 0, sizeof(int) * size);
	int i = 0;
	int minimal = INT_MAX;
	while (i < size) {
		seed[i] = getSystemTime() + i * 13;
		data[i] = r4_normal_01(&seed[i]) * (high - low) + low;
	//	data[i] = d[i] * (high - low) + low;
		if (minimal > data[i]) {
			minimal = data[i];
		}
		i++;
	...
	// if minimal is negative, then transfer it to positive.
	if (minimal < 0) {
		minimal = -minimal;
		i = 0;
		while (i < size) {
...
void freeIntNormalData(int * data) {
	free(data);
}
